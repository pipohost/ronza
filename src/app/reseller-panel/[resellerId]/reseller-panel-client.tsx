...
import { LogOut, Home, Edit, Calendar, Megaphone, Shield, Languages } from 'lucide-react';
...
function ResellerAnnouncements({ resellerId }: { resellerId: string }) {
    ...
    const texts = t.adminPanel.reseller.announcements;

    const handleSend = async () => {
        if (!message.trim()) {
            toast({ variant: 'destructive', title: texts.error, description: texts.emptyError });
            return;
        }
        setIsSubmitting(true);
        const result = await sendBulkAnnouncement(message, resellerId);
        if (result.success) {
            toast({ title: texts.success, description: texts.successDescription });
            setMessage('');
        } else {
            toast({ variant: 'destructive', title: texts.error, description: result.error });
        }
    ...
    return (
        <Card>
            <CardHeader>
                <CardTitle className="flex items-center gap-2"><Megaphone /> {texts.title}</CardTitle>
  ...
  return (
      <div className="container mx-auto py-10" dir={dir}>
        <div className="flex justify-between items-center mb-8">
          <div>
              <h1 className="text-3xl font-bold tracking-tight">{t.adminPanel.reseller.title} {reseller.name}</h1>
              <p className="text-muted-foreground">{t.adminPanel.reseller.description}</p>
          </div>
        ...
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
             <Card>
                <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                    <CardTitle className="text-sm font-medium">{t.adminPanel.reseller.stats.activeRooms}</CardTitle>
            ...
            <Card>
                <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                    <CardTitle className="text-sm font-medium">{t.adminPanel.reseller.stats.reservedNames}</CardTitle>
             ...
             <Card>
                <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                    <CardTitle className="text-sm font-medium">{t.adminPanel.reseller.stats.subscriptionStatus}</CardTitle>
                    <Calendar className="h-4 w-4 text-muted-foreground" />
                </CardHeader>
                <CardContent>
                    <div className="text-lg font-bold">{reseller.status}</div>
                    <p className="text-xs text-muted-foreground">{t.adminPanel.reseller.stats.renewsOn} {reseller.renewalDate}</p>
                </CardContent>
            </Card>
            <Card>
                <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                    <CardTitle className="text-sm font-medium">{t.adminPanel.reseller.stats.bannedUsers}</CardTitle>
        ...
        <Tabs defaultValue="rooms" className="mt-8">
          <TabsList className="grid w-full grid-cols-1 sm:grid-cols-5 md:w-auto">
            ...
            <TabsTrigger value="security">{t.adminPanel.tabs.security}</TabsTrigger>
