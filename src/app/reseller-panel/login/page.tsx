...
import { Terminal, Briefcase, Languages } from 'lucide-react';
import { useToast } from '@/hooks/use-toast';
import { verifyResellerPassword } from '@/app/actions/reseller-panel';
...
export default function ResellerPanelLoginPage() {
  ...
  const loginT = t.loginPages.reseller;
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
        ...
        const result = await verifyResellerPassword(resellerName, password, idToken);
        
        if (result.success && result.resellerId) {
            toast({
                title: loginT.successTitle,
                description: loginT.successDescription,
            });
            router.push(`/reseller-panel/${result.resellerId}`);
        } else {
        ...
        <form onSubmit={handleLogin}>
          <CardContent className="space-y-4">
            ...
            <div className="space-y-2">
              <Label htmlFor="resellerName">{loginT.resellerNameLabel}</Label>
              <Input
                id="resellerName"
                type="text"
                placeholder={loginT.resellerNamePlaceholder}
                value={resellerName}
