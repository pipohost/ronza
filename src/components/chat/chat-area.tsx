  ...
  return (
    ...
    >
      ...
      {Object.entries(allStreams).map(([userId, { stream, isLocal }]) => {
        if (hiddenVideos.has(userId)) return null;
        ...
        return (
          ...
          >
            ...
            <button onClick={() => hideVideo(userId)} className="absolute top-1 right-1 bg-black/50 p-1 rounded-full text-white z-30">
              <X className="h-4 w-4" />
            </button>
          </Rnd>
        )
      ...
      <div className="relative z-10 flex-1 flex flex-col overflow-y-auto bg-white/70 dark:bg-black/30 backdrop-blur-md min-h-0">
        <MessageList messages={messages} welcomeMessage={room.welcomeMessage} users={users} currentUserId={currentUserId} />
      </div>
    </div>
  );
}
